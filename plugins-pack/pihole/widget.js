...
(() => {
  ...
  var meta = {
    id: "pihole",
    name: "Pi-hole",
    description: "Pi-hole-v6-Statistik wie im Web-Dashboard (Anfragen, blockiert, Anteil, Domains auf Listen). Blocking per Klick. Daten via /api/plugins/pihole.",
    version: "1.2.0",
  ...
  function Widget({ config }) {
    ...
    const [blockBusy, setBlockBusy] = (0, import_react3.useState)(false);
    const fetch_ = (0, import_react3.useCallback)(async () => {
      ...
      try {
        const j = await pluginApiJson("pihole", "/", {
          method: "POST",
          cache: "no-store",
    ...
    const toggleBlocking = (0, import_react3.useCallback)(async () => {

...
function decryptAccountPassword(encrypted: string): string {
  try {
    ...
    if (/unable to authenticate|unsupported state/i.test(msg)) {
      throw new Error(
        'Gespeichertes Passwort kann nicht entschlüsselt werden — Konto bearbeiten und Passwort erneut speichern ' +
          '(nach Container-Neuaufbau oder geändertem SELFDASHBOARD_CALENDAR_KEY / fehlendem Volume für /app/data).',
      )
    }
    throw e
  }

...
// plugins-pack/home-assistant/server.ts
var dynamic = "force-dynamic";
var FETCH_TIMEOUT_MS = 12e3;
var MAX_ENTITIES = 25;
var ENTITY_ID_RE = /^[a-z_]+\.[a-z0-9_]+$/i;
function str(v) {
...
async function fetchState(base, id, token, signal) {
...
async function handlePost(req) {
  ...
  if (entities.length === 0) {
    return Response.json(
      { error: "missing_entities", detail: "Mindestens eine g\xFCltige entity_id (z. B. sensor.temperatur) angeben." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    for (const id of entities) {
      ...
      if (r.status === 401 || r.status === 403) {
        ...
        return Response.json(
          { error: "auth_failed", detail: "Long-Lived Access Token pr\xFCfen (Profil \u2192 Sicherheit)." },
          { status: r.status }
        );
      }
      if (r.status === 404) {
...
async function handleHomeAssistantPluginRequest(req, _path) {
  if (req.method !== "POST") return Response.json({ error: "method_not_allowed" }, { status: 405 });
  return handlePost(req);
}
function homeAssistantServerHandler(ctx) {
  return handleHomeAssistantPluginRequest(ctx.request, ctx.path);
}
export {
  homeAssistantServerHandler as default,
  dynamic
};

...
var sha256 = import_js_sha256.default.sha256;
var sha512256 = import_js_sha512.default.sha512_256;
var supported_algorithms = ["MD5", "MD5-sess", "SHA-256", "SHA-256-sess", "SHA-512-256", "SHA-512-256-sess"];
var parse = (raw, field, trim = true) => {
...
var DigestClient = class {
  constructor(user, password, options = {}) {
    this.user = user;
    this.hashFunc = import_md5.default;
    this.password = password;
    this.nonceRaw = "abcdef0123456789";
    this.logger = options.logger;
    this.precomputedHash = options.precomputedHash;
    let algorithm2 = options.algorithm || "MD5";
    if (!supported_algorithms.includes(algorithm2)) {
      if (this.logger) this.logger.warn(`Unsupported algorithm ${algorithm2}, will try with MD5`);
      algorithm2 = "MD5";
    }
    if (algorithm2.startsWith("SHA-256")) {
      this.hashFunc = sha256;
    } else if (algorithm2.startsWith("SHA-512-256")) {
      this.hashFunc = sha512256;
    }
    this.digest = { nc: 0, algorithm: algorithm2, realm: "" };
    this.hasAuth = false;
    const _cnonceSize = parseInt(options.cnonceSize);
    this.cnonceSize = isNaN(_cnonceSize) ? 32 : _cnonceSize;
  ...
  async getClient() {
    ...
    if (this._client == void 0) {
      const module2 = await Promise.resolve().then(() => __toESM(require_lib2(), 1));
      this._client = module2.default;
    }
    return this._client;
  }
  async fetch(url, options = {}) {
    const fetch2 = await this.getClient();
    if (this.basic) return fetch2(url, this.addBasicAuth(options));
    const resp = await fetch2(url, this.addAuth(url, options));
    if (resp.status == 401 || resp.status == this.statusCode && this.statusCode) {
      this.hasAuth = false;
      this.parseAuth(resp.headers.get("www-authenticate"));
      if (this.hasAuth) {
        const respFinal = await fetch2(url, this.addAuth(url, options));
  ...
  addBasicAuth(options = {}) {
    ...
    const auth = "Basic " + import_base_64.default.encode(this.user + ":" + this.password);
  ...
  addAuth(url, options) {
    ...
    const method2 = options.method ? options.method.toUpperCase() : "GET";
    ...
    const ha2 = this.hashWithAlgorithm(`${method2}:${uri}${_ha2}`);
...
var digest_fetch_src_default = DigestClient;

// plugins/fritzbox/lib/fritzboxTr064.ts
...
// plugins/fritzbox/lib/fritzTr064Shared.ts
...
function tr064OriginsForConnection(conn) {
  const root2 = fritzboxRootFromInput(conn.baseUrl);
  const u = new URL(root2);
...
// plugins/fritzbox/lib/tr064NodeFetch.ts
...
// plugins/fritzbox/lib/fritzHomeautoTr064.ts
...
async function homeautoCtx(conn, signal) {
  const client = new digest_fetch_src_default(conn.username || "", conn.password || "");
...
// plugins/fritzbox/lib/fritzEnergyImport.ts
...
// plugins/fritz-energy/server.ts
var runtime = "nodejs";
...
async function handleGet(req) {
  ...
  let root2;
  try {
    root2 = fritzboxRootFromInput(baseUrl);
  } catch {
    return Response.json({ ok: false, error: "bad_url" }, { status: 400 });
  }
  const key = energyStoreKey(root2, ain);
